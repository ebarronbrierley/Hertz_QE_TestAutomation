    ...
    public class Bonuses : BrierleyTestFixture
    {
        ...
        public void RealTime_Bonus_Positive(Member member, TxnHeader[] transactions, ExpectedPointEvent[] expectedPointEvents, string[] requiredPromotionCodes)
        {
            ...
            try
            {
                ...
                TestStep.Start($"Make AddMember Call for {member.Style} Member", "Member should be added successfully and member object should be returned");
                ...
                TestStep.Pass("API MemberDetails match passed MemberDetails", memberOut.GetMemberDetails(member.Style).ReportDetail());
                ...
                TestStep.Start($"Add {transactions.Length} transaction(s) to members virtual card with VCKEY = {memberVCKEY}", "Transaction(s) should be added to members virtual card");
                ...
                TestStep.Pass("Transaction(s) added to members virtual card", memberOut.VirtualCards.First().TxnHeaders.ReportDetail());
                ...
                TestStep.Start($"Verify TxnHeader(s) in DB with VCKEY = {memberVCKEY}", "TxnHeader(s) from database should match expected TxnHeader");
                ...
                TestStep.Pass("TxnHeader(s) from database matches expected TxnHeader(s)", transactions.Select(x => x.A_RANUM).ToArray());


                TestStep.Start($"Verify point transaction(s) exist for member VCKEY = {memberVCKEY}", "Point transaction(s) should exist for member");
